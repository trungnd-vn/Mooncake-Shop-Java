...
public class UpdateCakeController extends HttpServlet {
    ...
    private static final String ERROR = "updateCake.jsp";
    private static final String SUCCESS = "updateCake.jsp";
        ...
        try {
            ...
            if (!isMultiPart) {
                ...
                try {
                    ...
                    log("ERROR FileUploadException at UpdateCakeController: " + e.getMessage());
                    e.printStackTrace();
                }
                ...
                while (iter.hasNext()) {
                    ...
                    if (item.isFormField()) {
                        ...
                        try {
                            ...
                            log("ERROR ImageUploadException at UpdateCakeController: " + e.getMessage() + "ko quan tâm");
//                            e.printStackTrace();
                        }
                    }
                }
...
//                System.out.println(createdDate);
                Date createdDate_Add = new SimpleDateFormat("yyyy-MM-dd'T'HH:mm").parse(createdDate);
                Date expDate_Add = new SimpleDateFormat("yyyy-MM-dd'T'HH:mm").parse(expDate);
                Date updateDate_Add = new Date();
                String status = (String) params.get("cbxCakeStatus");
...
//                System.out.println(status + "HEllllllllllllllllllooooooooooooo");
                ...
                if (valid) {
                    if (categoryID != null) {
                        if (cakeName.length() != 0 || cakePrice > 0 || quantity > 0 || createdDate.length() != 0 || expDate.length() != 0) {
                            blo.updateCake(cakeID, cakeName, cakeImage, cakeDescription, cakePrice, quantity, createdDate_Add, expDate_Add, updateDate_Add, status, categoryID, accAdminUpdate);
                            request.setAttribute("NOTI1", "Update Successful!!!");
                        } else {
                            request.setAttribute("NOTI1", "Update UnSuccessful!!!");
                        }
                        url = SUCCESS;
                    }
                } else {
                    request.setAttribute("NOTI", "Update UnSuccessful!!!");
